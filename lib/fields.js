...
var forms = require('./forms'),
	...
	copy = function (original) {
		var copy = Object.keys(original).reduce(function (copy, key) {
			copy[key] = original[key];
			return copy;
		}, {});
        var field = new Field(copy);
        copy.field = field;
        return copy;
    };
Field.prototype.inspect = Field.prototype.toString = function () { return this.builder.toString(); };
...
exports.string = function (opt) {
    ...
    f.labelHTML = function (name, id) {
        ...
        var nameParts = name.replace(/\]/g, '').split(/\[/),
            labelName = nameParts[nameParts.length - 1],
        	label = '<label for="' + (id || 'id_' + name) + '"';
        ...
        label += '>' + this.labelText(labelName, id) + '</label>'
        return label;
    };
    f.classes = function () {
        var r = ['field'];
